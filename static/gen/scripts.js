...
var amount_stolen = 10000000000;

var stolen_by_second = amount_stolen / (20*365*24*60*60*10);
...
$(document).ready(function(){
	...
	setInterval(function(){
		$("#counter-money").text(Math.round(x++ * stolen_by_second));
	},100);
    ...
    for (offset in _ref) {
      ...
      if (window_position + 300 >= offset) {
        if (media.hasClass("illustration")) {
          ...
          iframe = $("<iframe></iframe>").attr("src", media.data("src")).attr("frameborder", 0).attr("scrolling", "no").attr("width", "100%").attr("height", "100%");
          media.html(iframe);
